...
function createWalls(cells, walls) {
	_.each(walls, function(wall) {
		cells[wall[0]][wall[1]] = elements.WALL;
	});
}

function Board(width, height, walls) {
	this.width = width;
	this.height = height;
	this.cells = create2dArray(width, height);
	createWalls(this.cells, walls);
	this.makeApple();
}
...
Board.prototype.makeApple = function(snakes) {
	var emptyCell = this.findEmptyCell(snakes);
	if (!emptyCell) {
		return false;
	}
...
Board.prototype.findEmptyCell = function(snakes) {	
	function match(pos) {
		return pos.x == x && pos.y == y;
	}

	function appleInTail(snake) { 
		return snake.path.some(match); 
	}	
	...
	for (var i = 0; i < 10000; i++) {
		...
		if (this.getCell(x, y) === elements.BOARD) {
			if (snakes && snakes.some(appleInTail)) {
				continue;
			}
			return { x: x, y: y };
		}		
