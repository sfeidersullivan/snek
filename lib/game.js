...
function getMove(game, player, opponent) {
	...
	if (controller) {
		try {
		    return controller.getNextMove(game, player, opponent);
		} catch (e) {
			player.status = "error";
			return;
		}
...
Game.prototype.tick = function () {
	...
	var moves = players.map(function (player) {
		var opponent = _.first(_.without(players, player));
		return getMove(this, player, opponent);
	}, this);
...
Game.prototype.makeApple = function() {
	this.board.makeApple(this.players.map(function(player) { return player.snake; }));
};
